...
     a) You may add the "transplant" function described in the CLRS book.

5. Implement the DB and use the unittest_db() function to test it. DO NOT CHANGE THIS CODE FOR YOUR SUBMISSION, as it will be used to test your code against the answers (with different input files).
...
6. Test your code against the provided input and output files.
...
7. Make sure your code has no memory leaks (using valgrind).

8. Your code should work beyond the provided unit tests. That is, even if it does work for all the given tests, if the code has an identifiable bug (i.e., by reading the source code), points WILL be deducted.
...
9. As before, complete the homework, and then push to GitHub. If you do not push to GitHub, the TA and I cannot see the code, and it will be considered a late assignment (i.e., not graded).
